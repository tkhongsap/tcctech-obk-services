...
const notificationAction = {
  ...
  findMessageType: async () => {
    try {
      showLoading();
      if (authenState.token.value) {
        const response = await OB_NOTI_SDK.client.messageCategoryShow(
          '',
          '',
          headerAccessToken(),
        );
  ...
  getAnnouncement: async () => {
    if (authenState.token.value) {
      const result = await OB_NOTI_SDK.client.messageAnonouncement(
        '',
        '',
        headerAccessToken(),
      );
  ...
  updateFCMToken: async () => {
    const result = await OB_NOTI_SDK.client.recipientFcmTokenUpdate(
      '',
      '',
      {
        push_token: {
