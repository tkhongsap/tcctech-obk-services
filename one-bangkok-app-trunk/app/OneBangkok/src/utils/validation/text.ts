...
export const TextValidation = {
  ...
  hasBlacklistCharacter: (text: string | undefined | null) => {
    return BLACKLIST_CHARACTERS.test(text || '');
  },
...
export function TextValidations(text: string | undefined | null) {
  ...
  if (/^\d+$/.test(text)) {
    ...
    const validateHasBlacklistCharacter =
      TextValidation.hasBlacklistCharacter(text);
    if (validateHasBlacklistCharacter) {
      return true;
    }
