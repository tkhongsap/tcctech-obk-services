...
import {AirQualityIndex, AirQualityIndexIndicator, AirQualityIndexIndicatorRange} from 'ob-bms-sdk/dist/api';
...
const AirQualityInfo = ({item}: {item: AirQualityIndexIndicator}) => {
  ...
  const formatDisplayRange = (ranges: AirQualityIndexIndicatorRange[]) => {
    const results = [];
    const joinWord = defaultSelected === 'th' ? ' หรือ ' : ' or ';
...
const AirQualitysInfo = ({
  ...
  const mapType = () => {
    ...
    switch (sensorType) {
      ...
      case 'AQI':
        return `${value} US AQI · ${indicators}`;
      case 'TVOC':
        return `${value} µg/m3 · ${indicators}`;
    }
  };
      ...
      <View className={getTheme('border-b border-line mx-3')}>
        <View className="flex w-full mt-6">
          <View>
            ...
            <Spacing height={20} />
            <Text weight="medium">
              {defaultSelected === 'th' ? 'สถานะ' : 'Indicator'}
            </Text>
          </View>
        </View>
        <FlatList
...
const AirQualityDetailInfoScreen = ({
  route: {
    params: {selectFloor, floorSensorData, selectType, towerName, towerId},
  },
}: Props) => {
  ...
  const defaultSelected = appLanguageActions.getLanguage();
  const getTitle = () => {
    if (defaultSelected === 'th') {
      return towerId !== 'OUT' ? 'ข้อมูลชั้น' : 'ข้อมูล';
    }
    return towerId !== 'OUT' ? 'Floor Details' : 'Details';
  };

  return (
    <View className=" bg-white h-full z-50">
      <Header title={getTitle()} leftAction="goBack" />
      <Spacing height={30} />

      <View className="flex-row px-4 w-full ">
        <HeadText
          tagline={`${t('General__One_bangkok', 'One Bangkok')} ${towerName}`}
          taglineColor={'vp-pass-date'}
          title={towerId !== 'OUT' ? selectFloor.name : ''}
        />
        <Spacing width={12} />
      </View>
      <Spacing height={12} />
