...
const WheelPickerForwardRef = forwardRef<FlatList, Props>((props, ref) => {
  const {
    selectedIndex,
    options,
    onChange,
  ...
  const handleMomentumScrollEnd = (
    ...
    if (index !== currentSelectedIndex) setCurrentSelectedIndex(index);
  ...
  useEffect(() => {}, [currentSelectedIndex]);
      ...
      <Animated.FlatList<string | null>
        ...
        renderItem={({item: option, index}) => (
          <WheelPickerItem
            key={`option-${index}`}
            index={index}
            option={mappingLanguageDay(option)}
            style={itemStyle}
            textStyle={
              currentSelectedIndex + visibleRest === index
                ? currentIndexItemTextStyle
                : itemTextStyle
            }
            height={itemHeight}
...
export default WheelPickerForwardRef;
