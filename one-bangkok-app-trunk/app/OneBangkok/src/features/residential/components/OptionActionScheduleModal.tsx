...
type Props = {
  initialSelectedSchedule: EventSchedule;
  schedules: EventSchedule[];
  onChanged?: (schedule: EventSchedule) => void;
};
...
const OptionActionScheduleModal = ({
  ...
  const [selected, setSelected] = useState<EventSchedule>(
    initialSelectedSchedule,
  );
  schedules.sort((a, b) => {
  ...
  const onPressCreate = () => {
    navigation.navigate('NewActionSchedule', {schedules: schedules});
    modalAction.hide();
  };
                ...
                <View
                  ...
                  <View className="flex-row " style={{gap: 12}}>
                    <Text weight="regular" size="B1" color="dark-gray">
                      {t(
                        'Residential__Home_Automation__Create_Actions_Schedule',
                        'Create Actions Schedule',
                      )}
                    </Text>
                  </View>
                </View>
