...
const CreateScheduleWeek = () => {
  ...
  const {weekSchedule} = useCoolingCreationState();
  const [getUpAt, setGetUpAt] = useState<Time>(weekSchedule.getUpAt.value);
  const [goToBedAt, setGoToBedAt] = useState<Time>(
    weekSchedule.goToBedAt.value,
  );
  ...
  const onPressNext = async (retry = 2) => {
    try {
      ...
      if (validTime) {
        ...
        navigation.navigate('CreateScheduleWeekDays');
      }
    } catch (error) {
      if (retry >= 1) {
  ...
  const navigateToErrorScreen = () => {
    navigation.navigate('AnnouncementResidentialScreen', {
      ...
      screenHook: 'CreateScheduleWeek',
      buttonColor: 'dark-teal',
      onPressBack: () => navigation.navigate('CreateScheduleWeek'),
    });
  };
          ...
          <View className="mb-4 px-4 flex flex-col" style={{gap: 12}}>
            <View className="flex flex-row items-center" style={{gap: 8}}>
              ...
              <Text size="H3" color="default" weight="medium">
                {t('Residential__Home_Automation__Week', 'Week')}
              </Text>
            </View>
            <View className="flex flex-col" style={{gap: 4}}>
              <Text size="B1" color="default" weight="medium">
                {t(
                  'Residential__Home_Automation__Week_Question_One',
            ...
            <View className="flex flex-col" style={{gap: 4}}>
              <Text size="B1" color="default" weight="medium">
                {t(
                  'Residential__Home_Automation__Week_Question_Two',
...
export default CreateScheduleWeek;
