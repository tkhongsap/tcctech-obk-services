...
const CreateScheduleSunday = () => {
  ...
  const [isHomeOnSunday, setIsHomeOnSunday] = useState<boolean | null>(
    scheduleSunday.isHomeOnSunday.value,
  );
  const [leaveHomeAt, setLeaveHomeAt] = useState<Time>(
    scheduleSunday.leaveHomeAt.value,
  );
  const [comeBackHomeAt, setComeBackHomeAt] = useState<Time>(
    scheduleSunday.comeBackHomeAt.value,
  );
  const [isLoading, setIsLoading] = useState(false);

  const validateTime = async () => {
    try {
      const {status, data} = await netatmoService.validateCrateSchedule([
        {
        ...
        {
          isHome: scheduleSaturday.isHomeOnSaturday.value,
          startTime: formatTime(
        ...
        {
          isHome: isHomeOnSunday,
          startTime: formatTime(
            isHomeOnSunday
              ? scheduleSunday.leaveHomeAt.hour.value
              : leaveHomeAt.hour,
            isHomeOnSunday
              ? scheduleSunday.leaveHomeAt.minute.value
              : leaveHomeAt.minute,
          ),
          endTime: formatTime(
            isHomeOnSunday
              ? scheduleSunday.comeBackHomeAt.hour.value
              : comeBackHomeAt.hour,
            isHomeOnSunday
              ? scheduleSunday.comeBackHomeAt.minute.value
              : comeBackHomeAt.minute,
          ),
        },
      ]);
      return {ok: status === 200, timetable: data?.timetable ?? []};
    } catch (error: any) {
  ...
  const navigateToErrorScreen = () => {
    navigation.navigate('AnnouncementResidentialScreen', {
      ...
      screenHook: 'CreateScheduleSunday',
      buttonColor: 'dark-teal',
      onPressBack: () => navigation.navigate('CreateScheduleSunday'),
    });
  };
          ...
          <View className="mb-4 px-4 flex flex-col" style={{gap: 12}}>
            <View className="flex flex-row items-center" style={{gap: 8}}>
              ...
              <Text size="H3" color="default" weight="medium">
                {t('Residential__Home_Automation__full_Sunday', 'Sunday')}
              </Text>
            </View>
            <View className="flex flex-col mb-3" style={{gap: 16}}>
              <Text size="B1" color="default" weight="medium">
                {t(
                  'Residential__Home_Automation__Sunday_Question_One',
              ...
              <View
                ...
                <TouchableOpacity
                  ...
                  onPress={() => setIsHomeOnSunday(false)}
                  disabled={isLoading}>
                  <Text
                    style={[
                      {
                ...
                <TouchableOpacity
                  className="flex flex-row items-center justify-center border border-line-light"
                  onPress={() => setIsHomeOnSunday(true)}
                  disabled={isLoading}>
                  <Text
                    style={[
                      {
            ...
            {isHomeOnSunday === false && (
              <>
                <View className="flex flex-col" style={{gap: 4}}>
                  <Text size="B1" color="default" weight="medium">
                    {t(
                      'Residential__Home_Automation__Sunday_Question_Two',
                ...
                <View className="flex flex-col" style={{gap: 4}}>
                  <Text size="B1" color="default" weight="medium">
                    {t(
                      'Residential__Home_Automation__Sunday_Question_Three',
...
export default CreateScheduleSunday;
