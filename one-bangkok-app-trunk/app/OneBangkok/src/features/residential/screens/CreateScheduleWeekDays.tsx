...
const CreateScheduleWeekDays = () => {
  const navigation = useNavigation();
  const {weekDays, weekSchedule} = useCoolingCreationState();
  const [isHomeDuringTheDay, setIsHomeDuringTheDay] = useState<boolean | null>(
    weekDays.isHomeDuringTheDay.value,
  );
  const [leaveHomeAt, setLeaveHomeAt] = useState<Time>(
    weekDays.leaveHomeAt.value,
  );
  const [comeBackHomeAt, setComeBackHomeAt] = useState<Time>(
    weekDays.comeBackHomeAt.value,
  );
  ...
  const onPressNext = async (retry = 2) => {
    try {
      ...
      if (validTime) {
        ...
        navigation.navigate('CreateScheduleWeekend');
      }
    } catch (error) {
      if (retry >= 1) {
  ...
  const navigateToErrorScreen = () => {
    navigation.navigate('AnnouncementResidentialScreen', {
      ...
      screenHook: 'CreateScheduleWeekDays',
      buttonColor: 'dark-teal',
      onPressBack: () => navigation.navigate('CreateScheduleWeekDays'),
    });
  };
          ...
          <View className="mb-4 px-4 flex flex-col" style={{gap: 12}}>
            <View className="flex flex-row items-center" style={{gap: 8}}>
              ...
              <Text size="H3" color="default" weight="medium">
                {t('Residential__Home_Automation__Weekday', 'Weekday')}
              </Text>
            </View>
            <View className="flex flex-col mb-3" style={{gap: 16}}>
              <Text size="B1" color="default" weight="medium">
                {t(
                  'Residential__Home_Automation__Weekday_Question_One',
              ...
              <View
                ...
                <TouchableOpacity
                  ...
                  onPress={() => setIsHomeDuringTheDay(false)}
                  disabled={isLoading}>
                  <Text
                    style={[
                      {
                ...
                <TouchableOpacity
                  className="flex flex-row items-center justify-center border border-line-light"
                  onPress={() => setIsHomeDuringTheDay(true)}
                  disabled={isLoading}>
                  <Text
                    style={[
                      {
            ...
            {isHomeDuringTheDay === false && (
              <>
                <View className="flex flex-col" style={{gap: 4}}>
                  <Text size="B1" color="default" weight="medium">
                    {t(
                      'Residential__Home_Automation__Weekday_Question_Two',
                ...
                <View className="flex flex-col" style={{gap: 4}}>
                  <Text size="B1" color="default" weight="medium">
                    {t(
                      'Residential__Home_Automation__Weekday_Question_Three',
...
export default CreateScheduleWeekDays;
