...
import {Screen, Header, Announcement} from '~/components';
  ...
  return (
    <View className={`p-6 ${getTheme('bg-light-gray')}`}>
      ...
      <View
        ...
        className={`w-full px-4 ${getTheme('bg-navy')}`}>
        <Button
          title={t('Signup__Signup_success__Button_primary', 'Setup 2FA')}
          className="text-[13px] text-left leading-tight w-full pl-1"
          rightIcon="next"
          rightIconColor="#fff"
          iconHeight={25}
          iconWidth={25}
...
const SignUpCompleteScreen = () => {
  ...
  useEffect(() => {
    logScreenView('SignUpCompleteScreen');
    assignPermissions();
  }, []);
