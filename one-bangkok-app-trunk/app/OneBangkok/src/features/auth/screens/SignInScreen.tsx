...
const goToMarcom = (navigation: StackNavigation) => {
...
const Login = async (
  ...
  try {
    ...
    if (hasToken && hasRefreshToken) {
      ...
      const parentScreen =
        routes.length >= 2 ? routes[routes.length - 2] : null;
      ...
      if (
        parentScreen &&
        bookmarkScreen.includes(parentScreen.name) &&
        navigation.canGoBack()
      ) {
        navigation.goBack();
      } else {
        goToMarcom(navigation);
      }

      await AsyncStorage.removeItem('tokenSustain');
    } else if (hasIdentity) {
      const identity = get(response, 'identity');
      const providerValue = get(identity, 'provider');
      const isEmailProvider = providerValue === 'email';
