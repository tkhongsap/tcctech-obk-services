...
import {FloatingStickyMenu} from '../components/FloatingMenu'; // Adjust the path based on where you place the component
...
const WhatsHappeningSection = ({
  ...
  data: WhatHappeningProps[];
  language: string;
}) => {
  const navigation = useNavigation();

  const [shouldRenderSection, setShouldRenderSection] = useState(false);
  useEffect(() => {
    const fetchEvents = async () => {
      await whatHappeningService.getEvents(language).then(response => {
  ...
  return (
    shouldRenderSection && (
      <View className={`px-5 py-10 ${getTheme('bg-muted-50')}`}>
        <View>
          <Text size="H3" className="font-medium text-black">
            {t('General__What_happening', 'What’s happening')}
          </Text>
          <Text size="B2" weight="medium" className="text-[#565656]">
            {t('General__Every_thing', 'Everything in one place')}
          </Text>
        </View>
        {data.length > 0 && (
          <>
            <Spacing height={32} />
            <WhatHappeningEvents data={data} />
            <Spacing height={32} />
          </>
        )}
        <Button
          onPress={() => navigation.navigate('AllEvents')}
          title={t('General__See_all_events', 'See all events')}
          ghost
          buttonHeight={55}
          rightIcon="arrowRightIcon"
          className="px-0 bg-red text-dark-gray"
        />
      </View>
    )
  );
...
const HomeScreen = () => {
  ...
  useScreenHook('WebSocket Elavator Home page', async event => {
    ...
    const unsubscribe = navigation.addListener('focus', () => {
      const initialFetchEvents = async () => {
        await whatHappeningService
          .suggestEvents(languageSelected)
    ...
    <Screen>
      ...
      <ScrollView
        ...
        <Pressable className={getTheme('bg-default')}>
          ...
          />
          {firebaseState?.enable_what_is_happening?.value && (
            <WhatsHappeningSection data={events} language={languageSelected} />
          )}
          <Spacing height={70} />
          <View className={'items-center'}>
