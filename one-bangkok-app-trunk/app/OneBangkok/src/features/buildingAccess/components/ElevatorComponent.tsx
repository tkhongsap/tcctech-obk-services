...
const DisableLocationAlway = () => {
  const enableAllowLocationWhileUsingTheApp =
    firebaseConfigState.enable_allow_location_while_using_app.value;
  return (
    <View className="flex flex-col items-center justify-center text-center mt-56">
      ...
      <View className="w-full h-16 px-6">
        <Spacing height={10} />
        <Text size="B1" weight="regular" color="muted" className="text-center">
          {enableAllowLocationWhileUsingTheApp
            ? t(
                'Building_access__Location_permission__Body',
                "Please allow “One Bangkok” to use device's location to call the elavator",
              )
            : t(
                'Building__access__Location__Alway__permission__Body',
                'Please allow "One Bangkok" to use device\'s location as "Always" allowed to call the elevator',
              )}
        </Text>
      </View>
    </View>
