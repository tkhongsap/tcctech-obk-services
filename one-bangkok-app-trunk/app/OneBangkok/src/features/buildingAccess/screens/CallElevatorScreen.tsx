...
const CallElevatorScreen = () => {
  ...
  const checkPermission = async () => {
    if (Platform.OS === 'ios') {
      const locationPermission = enableAllowLocationWhileUsingTheApp
        ? PERMISSIONS.IOS.LOCATION_WHEN_IN_USE
        : PERMISSIONS.IOS.LOCATION_ALWAYS;
      const permissionLocationaAlwayResult = await check(locationPermission)
        .then(result => {
          switch (result) {
      ...
      if (permissionLocationaAlwayResult !== RESULTS.GRANTED) {
        return;
      }
  ...
  useEffect(() => {
    logScreenView('CallElevatorScreen');
  }, []);
