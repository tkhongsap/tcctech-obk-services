...
import {restorePermissions} from '~/hooks/useAuthorization';
...
const SplashScreen = ({}: Props) => {
  ...
  const initConfig = useCallback(async () => {
    try {
      // Set defaults and fetch remote config
      await remoteConfig().setDefaults(remoteConfigDefaults);
      await withTimeout(remoteConfig().fetch(2), 5000).catch(activateError => {
        console.warn('Remote config fetch failed:', activateError);
        return false;
      });
      const fetchedRemotely = await withTimeout(
        remoteConfig().fetchAndActivate(),
        5000,
      ).catch(activateError => {
        console.warn('Remote config fetchAndActivate failed:', activateError);
        return false;
      });
      ...
      // Update Firebase config state
      firebaseConfigState[0].set({
        ...config,
        is_firebase: fetchedRemotely,
      });

      // Handle in-app messaging activation
      ...
      console.log(
        'Error: No configs were fetched from the backend, and the local configs were already activated',
      );
      firebaseConfigState[0].set({
  ...
  const preload = useCallback(() => {
    ...
    const goToHomeScreen = async (enable_marcom_mainpage: boolean) => {
      const showMyQrWhenLaunchesApp = await AsyncStorage.getItem(
        'showMyQrWhenLaunchesApp',
      );
      let route = [];
      if (!enable_marcom_mainpage) {
        route = [
          {
        ...
        route = [
          {
            name: 'MainPageScreen' as keyof RootStackParamList,
          },
        ];
      }
      ...
      if (authenState.token.value) {
        ...
        if (!enable_marcom_mainpage) {
          ...
          // switch future marcom
          navigation.dispatch(StackActions.replace('MainPageScreen'));
        }
      }
    ...
    const callback = (data: any) => {
      ...
      switch (type) {
        case WEBSOCKET_TYPE.LIFTCALLED:
          notifyScreenHook('WebSocket Elavator Main page', 'LIFTCALLED', data);
          notifyScreenHook('WebSocket Elavator Home page', 'LIFTCALLED', data);
          break;
        ...
        case WEBSOCKET_TYPE.VALET_STATUS_UPDATED:
          notifyScreenHook('WebSocket', 'VALET_STATUS_UPDATED', data);
          break;
        case WEBSOCKET_TYPE.NOTIFICATION_COUNTING_UPDATED:
          notifyScreenHook('WebSocket', 'NOTIFICATION_COUNTING_UPDATED', data);
          break;
        default:
          break;
      }
    ...
    initConfig().then(() => {
      console.log('is_firebase : ' + firebaseState.is_firebase);
      retrieveFCMToken();
          ...
          setAccount().then(async () => {
            ...
            if (semver.lt(version, appVersionUpdate)) {
              ...
              if (isWhiteListAllow && whiteListDeviceIds.includes(deviceId)) {
                goToHomeScreen(
                  firebaseConfigState[0].enable_marcom_mainpage.value,
                );
              } else {
