...
class SMSSender {
  ...
  public async send(data: PublishCommandInput) {
    if (
      this.client instanceof MockAWSClient ||
      this.client instanceof SNSClient
    ) {
...
class EmailSender {
  ...
  public async send(data: SendEmailCommandInput) {
    if (
      this.client instanceof MockAWSClient ||
      this.client instanceof SESClient
    ) {
...
export const emailSender = new EmailSender(
  process.env.EMAIL_SENDER_PROVIDER?.toUpperCase() as EmailClientType,
);
export const smsSender = new SMSSender(
  process.env.SMS_SENDER_PROVIDER?.toUpperCase() as SMSClientType,
);
