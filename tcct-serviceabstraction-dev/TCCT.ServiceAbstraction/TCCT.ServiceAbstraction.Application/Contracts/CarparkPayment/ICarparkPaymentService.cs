...
public interface ICarparkPaymentService
{
	Task<GetParkingDetailResult> GetParkingDetail(string search, bool lostcard);
	Task<InquiryPaymentTransactionResult> InquiryPaymentTransaction(string transactionno, string? subCode = null);
	...
	Task<GenerateReceiptResult> GenerateReceipt(string logid, int paymentid, string type, string? subCode = null);
}
