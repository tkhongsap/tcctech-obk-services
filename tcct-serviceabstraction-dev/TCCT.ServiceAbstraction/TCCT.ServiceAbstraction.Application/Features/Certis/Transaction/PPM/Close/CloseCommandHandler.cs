...
public class CloseCommandHandler : ICommandHandler<CloseCommand, CloseResult>
{
	...
	{
		var res = await _certisservice.Transaction.PPMService.CloseResult(request.WoId, request.ClosureComment, request.CompletionVerifiedBy, request.ClosureSignature, request.ClosedBy);
		return res;
	}
}
