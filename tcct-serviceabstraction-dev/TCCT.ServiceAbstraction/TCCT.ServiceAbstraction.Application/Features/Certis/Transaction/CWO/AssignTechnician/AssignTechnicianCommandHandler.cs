...
public class AssignTechnicianCommandHandler : ICommandHandler<AssignTechnicianCommand, AssignTechnicianResult>
{
	...
	{
		var res = await _certisservice.Transaction.CWOService.AssignTechnician(request.CwoId, request.AssignedBy, request.TechnicianId, request.OperatorNote, request.LocationId, request.Description, request.RequesterId, request.AssetId);
		return res;
	}
}
