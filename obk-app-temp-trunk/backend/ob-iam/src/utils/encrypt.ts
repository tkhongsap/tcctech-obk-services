...
export async function hashPassword(plainPassword: string | null) {
  try {
    if (plainPassword) {
...
export async function encryptWithPublicKey(publicKey: CryptoKey, data: object): Promise<ArrayBuffer> {
  ...
  const encryptedData = await crypto.subtle.encrypt({ name: "RSA-OAEP" } as RsaOaepParams, publicKey, new TextEncoder().encode(dataString));
...
export function arrayBufferToOutput(buf: ArrayBuffer, type: 'raw' | 'private' | 'public' | 'secret' = 'raw', outputFormat: 'pem' | 'string' = 'string'): string {
...
export async function importKeyFromPem(pem: string): Promise<CryptoKey> {
  ...
  return await crypto.subtle.importKey(isPrivateKey ? 'pkcs8' : 'spki', keyBuffer, { name: "RSA-OAEP", hash: "SHA-256" }, true, isPrivateKey ? ['decrypt'] : ['encrypt']);
}
