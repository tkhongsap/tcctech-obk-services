...
export class DateTimeUtils {
  ...
  static formatDate(value: string | number | Date, format = 'YYYY-MM-DD HH:mm:ss', timezone?: string): string {
    if (timezone) {
  ...
  static isBefore(date: string | number | Date, dateToCompare: string | number | Date): boolean {
    return dayjs(date).isBefore(dayjs(dateToCompare));
  }

  static isAfter(date: string | number | Date, dateToCompare: string | number | Date): boolean {
    return dayjs(date).isAfter(dayjs(dateToCompare));
  }

  static isSame(date: string | number | Date, dateToCompare: string | number | Date): boolean {
    return dayjs(date).isSame(dayjs(dateToCompare));
  }

  static getDayInWeekByweekDayName(day: (typeof DateTimeUtils.weekDayNames)[number]): number {
    return DateTimeUtils.weekDayNames.indexOf(day);
  }

  static getMonthInYearBymonthName(month: (typeof DateTimeUtils.monthInYearNames)[number]): number {
