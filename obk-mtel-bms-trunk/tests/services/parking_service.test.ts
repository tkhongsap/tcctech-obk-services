...
import { ParkingTowerRepository } from '../../src/repositories';
...
describe('Parking service', () => {
  ...
  describe('Fetch parking availability data', () => {
    ...
    it('should return true and delete old floor', async () => {
      jest.spyOn(FSParkingClient.httpClient, 'get').mockResolvedValueOnce({
        data: mockData,
      });
      const towerResult = await ParkingTowerRepository.create({
        data: {
          name: 'Building 1',
      ...
      await ParkingFloorRepository.create({
        data: {
          uid: '123',
          name: 'G',
          display_name: { en: 'G', th: 'G' },
          parking_towers: {
