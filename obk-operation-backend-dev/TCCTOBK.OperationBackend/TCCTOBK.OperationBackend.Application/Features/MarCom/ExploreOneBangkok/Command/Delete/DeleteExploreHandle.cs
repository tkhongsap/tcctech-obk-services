...
public class DeleteExploreHandler : IRequestHandler<DeleteExploreCommand, DeleteExploreResult>
{
	IUnitOfWork _uow;
	IDistributedCache _cache;
	public DeleteExploreHandler(IUnitOfWork uow, IDistributedCache cache)
	{
		_uow = uow;
		_cache = cache;
	}
	public async Task<DeleteExploreResult> Handle(DeleteExploreCommand request, CancellationToken cancellationToken)
	{
		DeleteEvent content = new DeleteEvent(request.Id);

		await _uow.MarcomRepository.DeleteExplore(content, request);
		...
		return new DeleteExploreResult() { StatusCode = StatusCodes.Status200OK, Message = "Success" };
	}
}
