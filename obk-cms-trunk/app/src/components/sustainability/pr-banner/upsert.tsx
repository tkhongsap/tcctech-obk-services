...
import {
  IBannerData,
  IBannerUpload,
} from '@src/services/sustainability/pr-banner/model'
...
import ContentBuilder from '../content-management/content'
...
const Upsert = (props: any) => {
  ...
  const [textBanner, setTextBanner] = useState<any>({
    en: undefined,
    th: undefined,
    cn: undefined,
  })
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      const uploadData: IBannerUpload = {
        fileName: file.name,
        fileContentBase64: base64Data,
        contentType: file.type,
      }

      const res = await bannerService.upload(uploadData)
    ...
    return (
      <div className='flex flex-column mt-5 gap-2'>
        ...
        <span className='flex text-xl font-bold text-primary-800'>
          {header ? `Header Image` : 'Hero Image'}
          {requiredLang.includes(mode) ? '*' : ''}
        </span>
        {invalidFile && (
          <Message
        ...
        <div className='flex flex-column gap-1 w-full'>
          <small className='text-xs text-primary-800'>
            {header
              ? 'Header image (Recommend image ratio 9:16) (Min 360x640px, Max 860x1527)'
              : 'Hero image (Recommend image ratio 9:16) (Min 360x640px, Max 860x1527)'}
          </small>
        </div>
        {fileUploadContent}
      </div>
  ...
  const getInit = async () => {
    await bannerService
      .initial()
      .then((res: any) => {
        setIsShowRelate(res?.data?.data?.isHasContent ?? true)
        formRef.current?.setValue(
          'relate',
          res?.data?.data?.isHasContent ?? true
        )
      })
      .catch((err) => {
        console.log('', err)
  ...
  useEffect(() => {
    if (bannerData) {
      ...
      if (bannerData.detail.th.title && bannerData.detail.th.title !== '') {
        reqLang.push('th')
      }
      if (bannerData.detail.cn.title && bannerData.detail.cn.title !== '') {
        reqLang.push('cn')
      }
  ...
  const isLangRequired = (lang: keyof OverallFileState, data: any) => {
    ...
    return (
      isEmptyString(title) &&
      isEmptyString(textBanner[lang]) &&
      !bannerState[lang].file &&
  ...
  }, [bannerState, textBanner, headerState, activeLang])
  ...
  const mapData = (data: any) => {
    ...
    result.detail.en.text = textBanner['en']
    result.detail.th.text = textBanner['th']
    result.detail.cn.text = textBanner['cn']
    ;(['en', 'th', 'cn'] as Array<keyof OverallFileState>).forEach(
      (lang: keyof OverallFileState) => {
        const { file: cover } = bannerState[lang]
  ...
  const onSubmit = (data: any) => {
    ...
    confirmDialog({
      message: 'Are you sure you want to save PR Banner?',
      closable: false,
      style: { width: '500px' },
        ...
        <div className='flex justify-content-start gap-3'>
          <Button
            ...
            onClick={async () => {
              ...
              setLoading(true)
              try {
                await toast.promise(bannerService.save(result), {
                  pending: 'Loading...',
                  success: 'Banner save successfully!',
                  error: {
                    render(err: any) {
                      const message =
        ...
        <div className='flex justify-content-start gap-3'>
          <Button
            ...
            onClick={async () => {
              ...
              setLoading(true)
              try {
                await toast.promise(bannerService.delete(params), {
                  pending: 'Loading...',
  ...
  const onInvalidSubmit = (err: any) => {
    ...
    languages.forEach((lang) => {
      if (err[`imageFile_${lang}`]) {
        updateState(setBannerState, lang)
      }
          ...
          <div className='card'>
            ...
            <div className='flex flex-column gap-5 p-5 lg:p-0 my-5'>
              <div className='flex flex-column'>
                ...
                <div className='flex flex-column gap-1 w-full'>
                  <small className='text-primary-800'>Banner name </small>
                  <TextField
                    name={`banner`}
                            ...
                            <div className='flex flex-column'>
                              ...
                              <div className='flex flex-column gap-1 w-full'>
                                <small className='text-primary-800'>
                                  Message ({lang.label}) (Recommend Wording Size
                                  : Heading 1 and White Color)
                                </small>
                                <div
                                  className='w-full'
                                  style={{
