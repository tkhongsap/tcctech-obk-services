...
import {
  ...
  convertCreateShowtimeItemsToBookingSlotTimes,
...
interface CreateShowtimeButtonProps {
  selectedDate: BookingSlotDate
  onUpdate: (id: string, value: BookingSlotTimes[]) => void
}

export function EditShowTimeButton({
  selectedDate,
  onUpdate,
  ...
  const handleClickEditShowtime = () => {
    const converted = convertBookingSlotTimeToShowtimeItem(
      selectedDate.bookingSlotTimes
    )
    setShowtimeItems(converted)
    setDates([selectedDate.slotDate])
  ...
  return (
    <>
      <Dialog
        header={
          <h4 className='tw-text-[#273281] text-1xl mb-0'>Edit showtimes</h4>
        }
        closable={false}
