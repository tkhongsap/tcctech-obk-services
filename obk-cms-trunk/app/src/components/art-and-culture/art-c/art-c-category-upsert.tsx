...
interface IArtCCategoryUpsert {
  type: string
  artCCategoryItem?: IArtCCategoryItem
}

interface IArtCCategoryFormTransValue {
  th: IArtCTypeForm
  en: IArtCTypeForm
  zh: IArtCTypeForm
}

const ArtCCategoryUpsert = ({
  type,
  artCCategoryItem,
}: IArtCCategoryUpsert) => {
  ...
  const [formTransValues, setFormTransValues] =
    useState<IArtCCategoryFormTransValue>({
  ...
  const initialFormValues = (item: IArtCCategoryItem | undefined) => {
    if (item && item.artCTranslation.length > 0) {
      ...
      setFormValues({
        ...value[formLocale as keyof IArtCCategoryFormTransValue],
        displayList: item.displayList || false,
        displayFreeLabel: item.displayFreeLabel || false,
        playlist: [],
        relateProgramIds: item.relateProgramIds || [],
  ...
  const handleFormLocaleChange = (locale: string) => {
    ...
    tempFormTransValues[formLocale as keyof IArtCCategoryFormTransValue] =
      formValues

    Object.keys(tempFormTransValues).forEach((key) => {
      if (key === formLocale) return
      tempFormTransValues[key as keyof IArtCCategoryFormTransValue] = {
        ...tempFormTransValues[key as keyof IArtCCategoryFormTransValue],
        displayList: formValues.displayList,
    ...
    const selectedLocaleValues =
      tempFormTransValues[locale as keyof IArtCCategoryFormTransValue]
    setFormValues({
  ...
  const onSubmit = async () => {
    ...
    if (!validationResult) {
      ...
      prepareFormValues[formLocale as keyof IArtCCategoryFormTransValue] =
        formValues
    }
    ...
    for (const key in formTransValues) {
      if (isOtherLocaleValid) {
        const checkFormValues = {
          title:
            formTransValues[key as keyof IArtCCategoryFormTransValue].title,
          shortDesc:
            formTransValues[key as keyof IArtCCategoryFormTransValue]
              .shortDesc || '',
          desc: formTransValues[key as keyof IArtCCategoryFormTransValue].desc,
          thumbnail:
            formTransValues[key as keyof IArtCCategoryFormTransValue].thumbnail,
          banner:
            formTransValues[key as keyof IArtCCategoryFormTransValue].banner ||
            '',
          ...
          externalLink:
            formTransValues[key as keyof IArtCCategoryFormTransValue]
              .externalLink || '',
          tags:
            formTransValues[key as keyof IArtCCategoryFormTransValue].tags ||
            [],
        }
        ...
        if (!isObjectDeepEqual(checkFormValues, initValues)) {
          ...
          if (!result) {
            ...
            prepareFormValues[
              key as keyof IArtCCategoryFormTransValue
            ].displayList = formValues.displayList
            prepareFormValues[
              key as keyof IArtCCategoryFormTransValue
            ].displayFreeLabel = formValues.displayFreeLabel
    ...
    let res = { id: artCCategoryItem ? artCCategoryItem.id : 0, status: false }
    for (const key in prepareFormValues) {
      if (formValuesItem.includes(key)) {
        ...
        tempAnswer.innerHTML =
          prepareFormValues[key as keyof IArtCCategoryFormTransValue].desc
        ...
        const saveValue = {
          ...prepareFormValues[key as keyof IArtCCategoryFormTransValue],
    ...
    if (res.status) {
      router.push(
        `/art-and-culture/art-c/category/${typeId}${
  ...
  const saveFormValues = async (
    ...
    try {
      ...
      if (type === 'create' && id === 0) {
        result = await artCCategoryServices.create(
          `${typeId}`,
          saveLocale,
          values
        )
      } else {
        result = await artCCategoryServices.edit(
          `${typeId}`,
          `${id}`,
          saveLocale,
          values
  ...
  const deleteItem = async (id: number) => {
    if (
      confirm(
        'Are you sure you want to delete this Art & Culture Sub Category?'
      )
    ) {
      setIsLoading(true)
      ...
      await artCCategoryServices
        ...
        .then(() => {
          router.push(
            `/art-and-culture/art-c/category/${artCCategoryItem!.artCTypeId}`
          )
        })
        .catch((error) => {
          setRequestError({
  ...
  const onBack = () =>
    router.push(`/art-and-culture/art-c/category/${router.query.typeId}`)
    ...
    <div className='flex gap-3 justify-content-center md:justify-content-start'>
      ...
      {type === 'update' && (
        <Button
          ...
          label='Delete this Art & Culture Sub Category'
          severity='danger'
          outlined
          disabled={isLoading}
          onClick={() => deleteItem(artCCategoryItem!.id)}
        />
      )}
      <Button
        type='button'
  ...
  const fetchContent = async (typeId: number) => {
    setIsLoading(true)
    await artCServices
      .get(`${typeId}`, 'all')
      .then((res) => {
  ...
  useEffect(() => {
    if (artCCategoryItem && artCCategoryItem.artCTypeId) {
      fetchContent(artCCategoryItem.artCTypeId)
    }

    fetchPrograms()

    setTimeout(() => {
      initialFormValues(artCCategoryItem)
    }, 250)
  }, [artCCategoryItem, locale])
              ...
              {formValues && (
                <div className='p-2'>
                  <div className='mb-5'>
                    ...
                    {type === 'update' && artCCategoryItem && (
                      <small className='font-medium'>
                        Last update&nbsp;
                        <span className='font-bold'>
                          {dayjs(artCCategoryItem.updatedAt).format(
                            'DD/MM/YYYY HH:mm'
                          )}
                          &nbsp;
                        </span>
                  ...
                  <div className='field grid mt-5'>
                    <div className='flex col-12 tw-items-center'>
                      ...
                      <InputSwitch
                        ...
                        onChange={(e) =>
                          setFormValues({ ...formValues, displayList: e.value })
                        }
                      />
                    </div>
                  </div>
                    ...
                    <div className='col-6 mb-2 px-2'>
                      ...
                      <div className='field grid pt-4'>
                        <div className='flex col-12 tw-items-center'>
                          ...
                          <InputSwitch
                            key={`displayFreeLabel-${formLocale}`}
                    ...
                    <div className='w-full flex-shrink-0 mt-5 px-2'>
                      ...
                      <div className='border-2 border-gray-200 tw-rounded-lg py-2 px-5'>
                        <div className='multi-select-display-all pt-4'>
                          <MultiSelect
                            key={`relateProgramIds-${formLocale}`}
                            display='chip'
                            value={
              ...
              >
                ...
                <div className='pt-4 px-4'>
                  <div className='text-sm' style={{ color: '#DC7032' }}>
                    <strong>{artCTypeTitle}</strong>
                  </div>
                  <h4 className='mt-2 mb-0'>{formValues.title || 'Title'}</h4>
                  ...
                  <div
                    ...
                    dangerouslySetInnerHTML={{ __html: formValues.desc }}
                  />
...
export default ArtCCategoryUpsert
