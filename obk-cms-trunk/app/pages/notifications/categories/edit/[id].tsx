...
import {
  ...
  ICategoryRequest,
} from '@src/services/notificationservice/category/model'
...
export default function EditCategory(props: Props) {
  ...
  const data = new CategoryRequestModel(props.data)

  const [errorState, setErrorState] = useState<number>(0)
  const [isSubmited, setIsSubmited] = useState<boolean>(false)

  useEffect(() => {
    setMenuName('Edit category')
  ...
  const onSubmit = async () => {
    ...
    value.visible = value.visibleDropdown === 1
    delete value._count
    delete value.id
    delete value.icon_id
    delete value.visibleDropdown

    const promise = OBNOTISDK.client
      .messageCategoriesUpdate(categoryid, value)
      .then(() => router.back())
      .finally(() => {
...
EditCategory.activePrime = true

export const getServerSideProps = withGenericServer(
  async (ctx: any) => {
    const { id } = ctx.props.query
    const res = await OBNOTISDK.client.messageCategoriesShow(id)
