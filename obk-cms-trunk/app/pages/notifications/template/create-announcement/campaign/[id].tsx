...
import {
  MappingCampaignToTemplateUIModel,
...
export default function AnnouncementEdit(props: Props) {
  OBNOTISDK.setBaseUrl(process.env.OB_NOTIFICATION_SDK_BASE_URL || '')

  const { categories, tags, data } = props
  ...
  const formData = new MappingCampaignToTemplateUIModel(data)
        ...
        >
          {
            {
              0: (
                <TemplateStep disableCategory={true} categories={categories} />
              ),
              1: (
                <MessageStep
...
AnnouncementEdit.activePrime = true
...
export const getServerSideProps = withGenericServer(
  ...
  {
    redirectPath: '/notifications/template/create-announcement',
  }
)
