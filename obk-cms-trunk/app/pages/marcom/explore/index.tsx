...
import PreviewTable from '@components/marcom/explore/previewTable'
...
const IconCell = ({
  ...
  value ? (
    <IconComponent className={className} style={{ color }} onClick={onClick} />
  ) : null

const ExploreOneBangkok = () => {
  ...
  const [filterExplore, debouncedFilterExplore, setFilterExplore] =
  ...
  const fetchHero = useCallback(
    async (paginationState: any) => {
      ...
      setIsLoading(true)
      try {
        const res = await exploreService.getAll(
          filterExplore,
          paginationState
            ? {
                ...paginationState,
    ...
    [debouncedFilterExplore]
  )

  useEffect(() => {
    fetchHero(tableState)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [debouncedFilterExplore, tableState])
        ...
        <div className='flex align-items-center justify-content-end'>
          <div className='flex gap-3 justify-content-center md:justify-content-start'>
            <Button
              ...
              label='Create new Explore'
              onClick={() => {
                router.push('/marcom/explore/create')
              }}
            />
          </div>
        </div>
...
export default ExploreOneBangkok
ExploreOneBangkok.activePrime = true
