import { Box } from '@chakra-ui/react'
import Upsert from '@components/sustainability/content-management/upsert'
import withGenericServer from '@hocs/server/generic'
import { PCODE } from '@src/data/constants/privilege'
import { contentService } from '@src/services/sustainability/content-mangement/service'
import React, { useEffect } from 'react'
import { useForm } from 'react-hook-form'

const EditContent = ({ initialData }: { initialData: any }) => {
  const defaultValue: any = {}
  const form = useForm({
    defaultValues: defaultValue,
  })

  useEffect(() => {
    if (initialData) {
      form.setValue('status', initialData.status ? '1' : '0')
      form.setValue('submenu', initialData.isSubMenu)
      form.setValue('relate', initialData.isShowRelatedLink)
      form.setValue('menu', initialData.parentId)

      form.setValue('menu_en', initialData.detail.en.menu)
      form.setValue('menu_th', initialData.detail.th.menu)
      form.setValue('menu_cn', initialData.detail.cn.menu)
      form.setValue('introduce_en', initialData.detail.en.introduce)
      form.setValue('introduce_th', initialData.detail.th.introduce)
      form.setValue('introduce_cn', initialData.detail.cn.introduce)

      form.setValue('titleRelated_en', initialData.detail.en.titleRelated)
      form.setValue('titleRelated_th', initialData.detail.th.titleRelated)
      form.setValue('titleRelated_cn', initialData.detail.cn.titleRelated)

      if (initialData.detail.en.cms.length > 0) {
        const contentEn = initialData.detail.en.cms.map((f: any) => {
          return {
            answer: f.text,
            type: f.contentType,
            fileName: f.fileName,
            imageURL: f.imageURL,
            originalFileName: f.originalFileName,
            youtube: f.youtubeURL,
          }
        })
        form.setValue('en_contents', contentEn)
      }

      if (initialData.detail.th.cms.length > 0) {
        const contentTh = initialData.detail.th.cms.map((f: any) => {
          return {
            answer: f.text,
            type: f.contentType,
            fileName: f.fileName,
            imageURL: f.imageURL,
            originalFileName: f.originalFileName,
            youtube: f.youtubeURL,
          }
        })
        form.setValue('th_contents', contentTh)
      }

      if (initialData.detail.cn.cms.length > 0) {
        const contentCn = initialData.detail.cn.cms.map((f: any) => {
          return {
            answer: f.text,
            type: f.contentType,
            fileName: f.fileName,
            imageURL: f.imageURL,
            originalFileName: f.originalFileName,
            youtube: f.youtubeURL,
          }
        })
        form.setValue('cn_contents', contentCn)
      }
    }
  }, [initialData])

  return (
    <Box maxW='inherit' pb='60px'>
      <Upsert
        formData={form}
        defaultValue={defaultValue}
        contentData={initialData}
        formType='edit'
      />
    </Box>
  )
}

export default EditContent

EditContent.activePrime = true

export const getServerSideProps = withGenericServer(
  async (ctx: any) => {
    const { id } = ctx.props.query

    let initialData: any = null

    if (id && typeof id === 'string') {
      initialData = await contentService
        .getContent(id)
        .then((res) => res.data)
        .catch((err) => {
          console.log('Fetch banner detail error: ', err)
          return null // Handle error appropriately
        })
    }

    return {
      props: {
        initialData,
      },
    }
  },
  {},
  {
    redirectPath: '/sustainability/all',
    accessPage: PCODE.EDITCONTENT,
  }
)
