...
import { errorObject, OBError } from "../utils/error_spec";
...
const newErrorHandler = (
  ...
  const message =
    err.serviceError?.message.replace("%s", get(err, "message", "")) ??
    OBError.OB_001.message;

  // Log the error
  // wait for implement log service
